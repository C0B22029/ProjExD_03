    ...
    while True:
        for event in pg.event.get():
            if event.type == pg.QUIT:
                return
            if event.type == pg.KEYDOWN and event.key == pg.K_SPACE:#キーが押されたらandキーの種類がスペースだったら
                beam = Beam(bird)
        ...
        if bomb is not None:
            bomb.update(screen)
        if beam is not None:#Noneではなければ実行する
            beam.update(screen)#ビームのアップデート　注意：pg.display.updateより上に書く
        pg.display.update()
        tmr += 1
        clock.tick(50)
